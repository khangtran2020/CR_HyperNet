...
def get_datasets(data_name, dataroot, normalize=True, bd_size=10000, use_embeddings=False):  # 10000
    ...
    train_size = len(dataset) - bd_size
    train_set, bd_set = torch.utils.data.random_split(dataset, [train_size, bd_size])

    return train_set, bd_set, test_set
...
def gen_classes_per_node(dataset, num_users, classes_per_user=2, high_prob=0.6, low_prob=0.4):
    ...
    for i in range(num_classes):
        probs = np.random.normal(0, 1, size=count_per_class)
...
def gen_random_loaders(data_name, data_path, num_users, bz, classes_per_user, use_embeddings):
    ...
    """
    print("Are we using embeddings in gen_random:", use_embeddings)
